...
class dimension_lexicon_builtin:
    def __init__(self, direction_of_interest, w2vmodel, min_count=1):
        ...
        elif self.direction_of_interest =='safe':
            self.pos_label="safe"
            self.neg_label="risky"
            self.all_pos_train = word_lists.safe_train['pos']
            self.all_neg_train = word_lists.safe_train['neg']
            self.all_pos_test= word_lists.safe_test['pos'] 
            self.all_neg_test= word_lists.safe_test['neg']

            self.pos_train, self.neg_train= clean_words_pairwise(word_lists.safe_train['pos'], word_lists.safe_train['neg'], self.w2vmodel, returnNA=False, min_count=min_count) 
            self.pos_test= clean_words(word_lists.safe_test['pos'] , self.w2vmodel, returnNA=False)
            self.neg_test= clean_words(word_lists.safe_test['neg'], self.w2vmodel, returnNA=False)
            
        elif self.direction_of_interest =='good':
            self.pos_label="good"
            self.neg_label="bad"
            self.all_pos_train = word_lists.good_train['pos']
            self.all_neg_train = word_lists.good_train['neg']
            self.all_pos_test= word_lists.good_test['pos'] 
            self.all_neg_test= word_lists.good_test['neg']
            
            self.pos_train, self.neg_train= clean_words_pairwise(word_lists.good_train['pos'], word_lists.good_train['neg'], self.w2vmodel, returnNA=False, min_count=min_count) 
            self.pos_test= clean_words(word_lists.good_test['pos'] , self.w2vmodel, returnNA=False)
            self.neg_test= clean_words(word_lists.good_test['neg'], self.w2vmodel, returnNA=False)
